...
public sealed record ProfileResult(MassifProfile? Profile, string SourcePath, string Output)
{
...
public static class SourceProfiler
{
    public static async Task<ProfileResult> RunAsync(string sourcePath, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (!build.Ok)
                return new ProfileResult(null, sourcePath, build.Output);
            ...
            if (!File.Exists(massifOut))
                return new ProfileResult(null, sourcePath, run.Output);
            ...
            return new ProfileResult(profile, sourcePath, build.Output);
        }
        finally
        {
